...
integer :: pressure_solver = 0 ! 0 = pressure_big, 1 = pressure_orig
...
logical:: doShipTrackConditionals = .false. !cloud ship/no ship, clear ship/no
...
real :: tvpr_wtg(nz) ! for statistics output (buoyancy temperature anomaly wrt reference profile in K)

logical :: dowtg_blossey_etal_JAMES2009 = .false. ! alternate implementation
logical :: lambda_ship_track = .false. ! alternate implementation
logical :: lambda_cf = .false. ! alternate implementation
logical :: print_lambda = .false. ! alternate implementation
...
real :: lambda_wtg_min = 10.e3 ! Minimum lambda_wtg for lambda_ship_track=true.
real :: lambda_ship_track_rate = 61.5e3 ! Lateral dispersion rate of ship plume. default = 61.5 (61.5 km per day)
real :: lambda_ship_track_start_time = 196.75 ! lambda start to increase from this time (in day) 
real :: wtg_start_time = 0. ! start time of WTG forcing in calendar days
real :: wtg_start_ramp = 0.03125 ! ramp down WTG forcing over this time period (in days)
real :: wtg_end_time = 1.e8 ! end time of WTG forcing in calendar days
real :: wtg_end_ramp = 0.03125 ! ramp down WTG forcing over this time period (in days)
real :: pres_trop = 1.e4 ! ramp down WTG forcing over this time period (in days)
...
integer :: nsaveMSE = 1000 ! output budgets every nsaveMSE timesteps.  Must be a multiple of nstat.
...
!bloss(2019-01-22): transient nudging, which switches on 
!  between transient_nudging_start and transient_nudging_end.
!  Nudging has fixed 600 second timescale aloft and specified timescale in BL
!  It is applied whenever/wherever it is stronger than the background nudging
logical :: donudging_transient = .false.
real :: transient_nudging_start = -11. ! day
real :: transient_nudging_end = -10. ! day
real :: transient_nudging_ramp = 0.0625 ! day
real :: tau_transient_nudging = 600. ! second
...
! outputs of variables involved in surface flux computations.
!   NOTE: A bunch of scalar variables will be output as a profile in OUT_STAT/
real :: surffluxstats(nzm)

! Options for the treatment of large-scale horizontal advection of q and T
! Relaxational large-scale horizontal advection, following Randall & Cripe (1999)
!   The diagnosed large-scale advection is used to derive "upwind" soundings
!   of q and T, e.g., q_upwind = q_obs + tau*hadvq, 
!   where tau = hadv_length_scale/wind_speed_obs.  Then, the horizontal advection
!   is computed as hadvq_upwind = (q_upwind - q0)/tau.  This approach can be interpreted
!   as applying both the prescribed hadvq and havdt, with an additional nudging term
!   whose timescale is tau, as defined above.
logical:: doUpwindHorizontalAdvection =.false.
real:: hadv_length_scale=200.e3  
real :: qtend_obs(nzm), ttend_obs(nzm) ! original input values of hadvq and hadvt

! Alternately, the Limit horizontal advection so that it will not worsen existing 
!   model biases in q or T.  If q0 (the model's domain mean qt) is moister than
!   the upwind sounding implied by q_obs and hadvq (as specified above), limit 
!   positive values of hadvq, so that the horizontal advection will not worsen that
!   existing bias.  The rationale here is that the LES model is moister than the
!   upwind region in the reanalysis used to compute hadvq, then the renalysis hadvq
!   should not be permitted to further moisten the LES model.
logical:: doLimitHadvUsingUpwindValue = .false.
