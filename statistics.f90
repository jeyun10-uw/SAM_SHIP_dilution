...
use micro_params, only: shipv2_time0, NC_env, NAc_thresh, NAc_thresh_max
...
! UW ADDITIONS
 ...
 integer, parameter :: n_avg_lev = 20 !num lvls to avg. aerosol over
 ...
 real, dimension(nx,ny) :: height_inv, height_inv_offset !array of inversion base heights
...
!mcmichael: added conditional statistics for ship track stats 
                        ...
                        jloop3: do j = 1,ny
                               kloop3: do k = h_cb,nzm
                                         ...
                                         if (der_temp.gt.der_thresh) then
                                             height_inv(i,j) = k !inversion height
                                             height_inv_offset(i,j) = k - nz_offset !20 grid spaces below inversion height
                                             if (der_temp.gt.der_thresh) exit kloop3
                                         else
                                         !for cases where no inv. base was found
                                         height_inv(i,j) = 0
                                         height_inv_offset(i,j) = 0
                                         endif
                               enddo kloop3
                               der_temp = 0.
                        enddo jloop3
         ...
         !------------------------------------------------------------------
           ...
           if(day.lt.shipv2_time0) then
              ...
              NAc_thresh_max = 0.
                ...
                !calculate near-surface layer average NAc in each column
                ...
                !            aero_xy_col(i,j) = aero_xy_col(i,j) + aero_xy(i,j)*(z_diff1(k)/tot_depth)
                !         enddo
                !   enddo
                !enddo
                ...
                edge_factor(:) = 0.
                do i = 1,nx
                   if(tmpx_pos(i).le.((dx*float(nx_gl))*edge_frac_max).OR.tmpx_pos(i).ge.((dx*float(nx_gl))*(edge_frac_min))) then
                ...
                !determine number of global subdomains for max value average
                is_edge_task = .false.
                ...
                !sum number of is_edge_task = .true. occurences
                if(dompi) then
                  buffer30(1) = merge(1.0, 0.0, is_edge_task) !convert logical to int
                  call task_sum_real(buffer30,buffer31,1) !global sum
                  total_edge_tasks = buffer31(1)
                end if
                ...
                !sum all of the subdomain maximum values
                if(dompi) then
                ...
                do k = 1,nzm
                   ...
                   do j = 1,ny
                      do i = 1,nx
                         ...
                         if((z(k).lt.z(nz_offset))) then
                         !if((z(k).lt.z(height_inv_offset(i,j))).and.(height_inv(i,j).ne.0)) then
		...
		if(doShipDilution.and.doAutoDilutionStart) then
		  ...
		  ENDIF
                ...
                NAc_thresh_max = edge_thresh + fixed_delta

                if(masterproc) then
                   print*, 'NC in environment (#/mg) =', NC_env*1.e-6
                   print*, 'NAc in environment (#/mg) =', NAC_mean2*1.e-6
                   print*, 'NAc in environment (max column method) (#/mg) =', edge_thresh*1.e-6
                   print*, 'NAc standard deviation (#/mg) =', NAC_std2*1.e-6
                   print*, 'Total edge tasks =', total_edge_tasks
                endif            
		  ...
		  !mcmichael: only loop through i,j indices for ship track stats
			...
			!if any liquid is present = .true.
                  	...
                  	if(day.lt.shipv2_time0) then
                     		...
                     		condition_track = aero_xy_col(i,j).gt.NAc_thresh_max.and.NAc_thresh_max.ne.0.
                  	endif

                  	!Print out aerosol threshold to log file
                  	if(masterproc.and.(i+j+k.eq.3)) then 
                    		print*, 'Aerosol threshold for ship track (#/mg) =', NAc_thresh_max*1.e-6
                  	endif
