...
subroutine wtg_james2009(nzm, pres, tabs_ref, qv_ref, tabs_model, &
     qv_model, qcond_model, f_coriolis, lambda_wtg, am_wtg, am_wtg_exp, pres_trop, &
     omega_wtg, tvprime_wtg, ktrop)
  implicit none
  ...
  real, intent(out) :: tvprime_wtg(nzm) ! virtual temperature anomaly wrt reference sounding in K
  ...
  if(pres(2).gt.pres(1)) then
    ...
    call wtg_james2009_driver(nzm, pres_local, tabs_ref_local, qv_ref_local, tabs_model_local, &
         qv_model_local, qcond_model_local, f_coriolis, lambda_wtg, am_wtg, am_wtg_exp, pres_trop, &
         omega_wtg, tvprime_wtg, ktrop)
    ...
    ! re-order tvprime_wtg to be indexed from top down.
    tmp(1:nzm) = tvprime_wtg(1:nzm)
    tvprime_wtg(1:nzm) = tmp(nzm:1:-1)
  ...
  if(debug) then
    do k = 1,nzm
      write (*,992) tabs_ref(k), tabs_model(k), omega_wtg(k)
      992 format('Tabs (ref/model) = ',2F10.2,' omega = ',E12.4)
    end do
  end if
...
contains
  ...
  subroutine wtg_james2009_driver(nzm, pres, tabs_ref, qv_ref, tabs_model, &
    ...
    real, intent(in) :: am_wtg     ! WTG momentum damping rate in 1/s at p/pref (default = 1./86400. /s)
    real, intent(in) :: am_wtg_exp ! exponenent in WTG momentum damping rate, dimensionless (default = 1.)
    real, intent(in) :: pres_trop ! exponenent in WTG momentum damping rate, dimensionless (default = 1.)
    ...
    ! save virtual temperature anomaly for output
    tvprime_wtg(1:nzm) = tv_model(1:nzm) - tv_ref(1:nzm)
