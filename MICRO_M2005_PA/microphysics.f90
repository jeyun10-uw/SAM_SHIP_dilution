...
! original implementation and updates/fixes by Peter Blossey, UW, 2007-
! - single (accumulation) mode aerosol implementation by Andy Berner, UW, ~2010-2013
!   Published in Berner et al (2013, ACP, https://doi.org/10.5194/acp-13-12549-2013).
!   Includes scavenging of unactivated aerosol by cloud droplets and rain based on 
!   a scheme developed by Andreas Muhlbauer and described in the appendix of Berner et al (2013).
! - Updates backported from Matt Wyant's Two mode mode aerosol implementation, ~2020-2022
!    + scavenging of unactivated aerosols is now based on the swollen diameter due to water vapor uptake.
!    + The accumulation mode (both activated and unactivated) is treated as a single lognormal
!     size distribution, in which activated aerosols are assumed to be larger than unactivated 
!     aerosols.
...
use vars, only: pres, rho, dtn, w, t, tabs, qv, qcl, qpl, qci, qpi, &
     ...
     t2leprec, q2leprec, twleprec, qwleprec, &
     nrainy, nrmn, ncmn, &
...
integer, allocatable, dimension(:) :: flag_advect, flag_nudging
...
real, allocatable, dimension(:,:) :: & ! statistical arrays
     ...
     mknudge, & ! nudging tendency, requires mkobs(:,n), mk0(:,n) and flag_nudging(n)==1
     ...
     mk0, & ! mean profiles 
     mkobs, & ! observational profiles (can be used for nudging)
     mk_ref
...
!bloss      do_micro3dout         ! flag to output 3d snapshot for a bunch of micro tendencies
  ...
  if(doprogaerosol) then
    ...
    if(doicemicro) then
      if(masterproc) write(*,*) 'doicemicro must be .false. for doprogaerosol to be used.'
      if(masterproc) write(*,*) 'Aerosols within frozen hydrometeors are not tracked in the code at present.'
!!$      if(masterproc) write(*,*) '.... Stopping now ....'
!!$      if(masterproc) write(*,*) 'If you want to run the aerosol microphysics with ice processes, '
!!$      if(masterproc) write(*,*) '  comment out the task_abort() just below and uncomment the warning message.'
!!$      call task_abort()
      if(masterproc) write(*,*) '.... WARNING -- COULD LEAD TO INCONSISTENCIES DUE TO FREEZING OF ....'
      if(masterproc) write(*,*) '....   OF HYDROMETEORS BEING NEGLECTED IN THE AEROSOL  ....'
    end if
   ...
   if(doMinimumActivatedDropSize.and.masterproc) then
     write(*,*) '========================================================================'
...
!bloss(2020-11):        nmicro_fields = nmicro_fields + 3 !brnr add single wet and dry prognostic aerosol modes with 2 moments
  ...
  if(.not.isallocatedMICRO) then
     ...
     allocate(micro_field(dimx1_s:dimx2_s,dimy1_s:dimy2_s,nzm,nmicro_fields), &
          ...
          mknudge(nzm,nmicro_fields), & !bloss(2022-01)
    ...
    ! initialize flag arrays to all mass, no number, no precip
     ...
     if(doprogaerosol.AND.(aerinitmode.eq.0)) then
 ...
 ! compute initial profiles of liquid water - M.K.
     ...
     if(doprogaerosol) then
       ...
       case(0)
         ...
         ! chun: ref profiles for ship dilution calc
         mkobs(1:nzm,inaccr) = Na_accum_ref(1:nzm)
         mkobs(1:nzm,iqaccr) = qa_accum_ref(1:nzm)
         ...
         do k = 1,nzm
           do j = 1,ny
             do i = 1,nx
               ...
               ! chun ref profiles for ship dilution
               micro_field(i,j,k,inaccr) = Na_accum_ref(k)
               micro_field(i,j,k,iqaccr) = qa_accum_ref(k)
             end do
           end do
         end do
...
real, dimension(nzm) :: &
     tmpqcl, tmpqci, tmpqr, tmpqacc, tmpqacc_ref, tmpqad, tmpqaw, tmpqar, tmpqs, tmpqg, tmpqv, &
     tmpncl, tmpnci, tmpnr, tmpnacc, tmpnacc_ref, tmpnad, tmpns, tmpng, tmpzero, tmpcmd, tmpcmddry, wet_to_dry_diam_ratio, tmprhw, &
     tmpnuc, tmpnur, tmpw, tmpwsub, tmppres, tmpdz, tmptabs, &
     tmtend1d, &
     mtendqcl, mtendqci, mtendqr, mtendqad, mtendqaw, mtendqar, mtendqacc, mtendqs, mtendqg, mtendqv, &
     mtendncl, mtendnci, mtendnr, mtendnad, mtendns, mtendng, mtendnacc,  &
...
!!$                    if ((tmprhw(k).GT.0.99).AND.(tmpqcl(k).GT.0.0005)) then
                  ...
                  if ((tmpnad(k).gt.0.).AND.(tmpqad(k).gt.0.)) then
                    ...
                    wet_to_dry_diam_ratio(k) = MAX(1., 2.*rwet(1)/ddry)
                    tmpcmddry(k) = ddry
                    if((nstep.eq.1).AND.(icycle.eq.1).AND.(i+j.eq.2).AND.masterproc) then
                      write(*,942) k, tmpqcl(k), tmprhw(k), tmpcmddry(k), &
                           wet_to_dry_diam_ratio(k)*tmpcmddry(k)
                      942 format('k/QCL/RHW/DryDiam/WetDiam = ',I4,4e12.4)
                    end if
                  else
                    wet_to_dry_diam_ratio(k) = 1. ! default to one
                    tmpcmddry(k) = 1.e-9
                  end if
                end do
                ...
                if(DoScavengeAerosolDryDiameter) wet_to_dry_diam_ratio(:) = 1.

                ! compute scavenging due to cloud
                if (doscavcloud2m.eq..true.) then
                  tmpcmd(:) = tmpcmddry(:)*wet_to_dry_diam_ratio(:)
      ...
      ! compute scavenging due to rain
            ...
            do k = 1, nzm
              !bloss(2019-07-22): check for qad>0 and Nad>0
              if ((tmpnad(k).gt.0.).AND.(tmpqad(k).gt.0.)) then
                tmpcmddry(k) = EXP(-1.5*LOG(aer_sig1)**2.)*(6.*tmpqad(k)/&
                     (pi*rho_aerosol*tmpnad(k)))**(1./3.)
              else 
                tmpcmddry(k) = 1.e-9
              end if
...
!bloss        end if
        ...
        !bloss(2022-03/BUGFIX): Previously, the tendencies were only
        !  stored if do_scav_3d_output==true.  This would mean that
        !  the scavenging tendencies were incorrectly zeroed out
        !  when do_scav_3d_output==false. and icycle>1 (i.e. for
        !  all substeps but the first).
...
do n = 1,nmicro_fields
  if (ANY(isnan(micro_field(:,:,:,n)))) then
    write(*,*) 'NaN found in ', TRIM(mkname(n)), ' on task ', rank
    loc_nan(1) = 1
    STOP ' in micro_proc() because of NaN'
  end if
end do
...
real omn, omp, tmp(nzm,nmicro_fields), tmp_profile(nzm)
integer i,j,k,n
...
!bloss: Placeholder for defining these quantities properly at a later date
t2leprec(:) = 0.
twleprec(:) = 0.
q2leprec(:) = 0.
...
do n = 1,nmicro_fields
  ...
  if(n.eq.iqv) then
   ...
   if(flag_advect(n).eq.1) then
     ...
     ! add tendency due to large-scale advection
     name = trim(mkname(n))//'LSADV'
     longname = 'Tendency of '//trim(mklongname(n))// &
          ' due to large-scale advection'
     units = trim(mkunits(n))//'/day'
     call add_to_namelist(count,microcount,name,longname,units,0)
     ...
     if((n.ne.index_water_vapor).AND.(flag_nudging(n).eq.1)) then
       ...
       ! add field for observations used to nudge field
       name = trim(mkname(n))//'_OBS'
       longname = 'Observed value of '//trim(mklongname(n))// &
            ' (used for nudging)'
       call add_to_namelist(count,microcount,name,longname,units,0)
     end if
...
!bloss      call add_to_namelist(count,microcount,name,longname,units,0)
   ...
   !bloss: Uncomment this if the aerosols have large-scale 
   !   horizontal advective tendencies.
!!$   if((n.eq.inacc).OR.(n.eq.iqacc)) then
!!$      name = trim(mkname(n))//'_VADV'
!!$      longname = 'Large-scale vertical advection of '//trim(mklongname(n)) 
!!$      units = TRIM(mkunits(n))//'/day'
!!$      call add_to_namelist(count,microcount,name,longname,units,0)
!!$    end if
  ...
  if(n.eq.iqv) then
   ...
   if(flag_advect(n).eq.1) then
     ...
     if((n.ne.index_water_vapor).AND.(flag_nudging(n).eq.1)) then
       ...
       call hbuf_put(trim(mkname(n))//'_OBS', &
            mkobs(:,n),mkoutputscale(n))
     end if
...
function Get_cloudaerosol() !mcmichael: cloud aerosol concentration
  real, dimension(nx,ny,nzm) :: Get_cloudaerosol
  Get_cloudaerosol = micro_field(1:nx,1:ny,1:nzm,incl)
end function Get_cloudaerosol

function Get_accumaerosol() !mcmichael: accumulation mode aerosol concentration
  real, dimension(nx,ny,nzm) :: Get_accumaerosol
  Get_accumaerosol = micro_field(1:nx,1:ny,1:nzm,inacc)
end function Get_accumaerosol
