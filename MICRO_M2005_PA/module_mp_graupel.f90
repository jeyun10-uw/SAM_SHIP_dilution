...
use params, only: lcond, lsub, cp, rgas, rv, track_spreading_rate, doShipDilution, use_scam_track_width_spreading_rate ! chun: track_spreading_rate is added
   use grid, only: dx, day, masterproc, nstep, nstat
...
! MODE 2
         ...
         SIG2 = aer_sig2
         NANEW2 = 0. !bloss: placeholder for future second dry aerosol mode aer_n2
!bloss         RM2 = 1.3E-6
!bloss         SIG2 = 2.5
!bloss         NANEW2 = 1.E6
...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
SUBROUTINE M2005MICRO_GRAUPEL(QC3DTEN,QI3DTEN,QNI3DTEN,QR3DTEN,QAD3DTEN,QAW3DTEN,QAR3DTEN,QACC3DTEN, &
       NC3DTEN,NI3DTEN,NS3DTEN,NR3DTEN,NAD3DTEN,NACC3DTEN,QC3D,QI3D,QNI3D,QR3D,QAD3D,QAW3D,QAR3D,QACC3D,QACC_REF, &
       NC3D,NI3D,NS3D,NR3D,NAD3D,NUC3D,NUR3D,NACC3D,NACC_REF,T3DTEN,QV3DTEN,T3D,QV3D,PRES,RHO,DZQ,W3D,WVAR,PRECRT,SNOWRT, &
...
!brnr: make tendencies for aerosol mass available
      ...
      REAL, DIMENSION(KMS:KME) ::  QACC3DTEN          ! chun: Ship dilution: ACCUM AEROSOL MASS MIXING RATIO TENDENCY (KG/KG/S)
...
!brnr: make tendencies for aerosol number available
      ...
      REAL, DIMENSION(KMS:KME) ::  NACC3DTEN          ! Chun: Ship dilution: ACCUM AEROSOL NUMBER CONCENTRATION TENDENCY (1/KG/S)
...
!brnr: add mass for wet (rain and cloud) and dry aerosol     
      ...
      REAL, DIMENSION(KMS:KME) ::  QACC3D             ! Chun REF for SHIP dilution: Accumulation-mode AEROSOL MASS MIXING RATIO (KG/KG)
      REAL, DIMENSION(KMS:KME) ::  QACC_REF           ! Chun REF for SHIP dilution: Accumulation-mode AEROSOL MASS MIXING RATIO (KG/KG)
...
!brnr: add number for wet and dry aerosol
      ...
      REAL, DIMENSION(KMS:KME) ::  NACC3D             ! REF for SHIP dilution: Accumulation-mode AEROSOL NUMBER CONCENTRATION (1/KG)
      REAL, DIMENSION(KMS:KME) ::  NACC_REF           ! REF for SHIP dilution: Accumulation-mode AEROSOL NUMBER CONCENTRATION (1/KG)
...
!brnr: make sedimentation tendencies available for aerosol
      ...
      REAL, DIMENSION(KMS:KME) :: NISTEN !bloss(2020-10): complete set of sed. tendencies
      REAL, DIMENSION(KMS:KME) :: NSSTEN !bloss(2020-10): complete set of sed. tendencies
      REAL, DIMENSION(KMS:KME) :: NGSTEN !bloss(2020-10): complete set of sed. tendencies
...
! MODEL INPUT PARAMETERS (FORMERLY IN COMMON BLOCKS)
        ...
        REAL, DIMENSION(kms:kme,n_proc_extra), INTENT(INOUT) :: proc_extra
...
! SIZE PARAMETER VARIABLES
     ...
     integer, intent(in) :: n_tendency
     real, dimension(kms:kme,n_tendency), intent(out) :: stend
...
! MICROPHYSICAL PROCESSES
     ...
     REAL, DIMENSION(KMS:KME) ::  PCCN      ! CHANGE Q DROPLET ACTIVATION  !bloss(NOTE): This is never used...
...
! BRNR ADDED 2/13/12
!bloss: These first six tendencies produced by the limiters correspond to real physical
!         processes: cloud and rain evaporation.  I am re-naming the tendencies
!         so that they can be interpreted more physically.
     REAL, DIMENSION(KMS:KME) ::  NSUBCLIM  ! LOSS OF NC DUE TO EVAPORATION OF CLOUDS IN CELLS WITH VERY SMALL QC !bloss
     REAL, DIMENSION(KMS:KME) ::  NSUBRLIM  ! LOSS OF NR DUE TO EVAPORATION OF RAIN IN CELLS WITH VERY SMALL QR !bloss
     REAL, DIMENSION(KMS:KME) ::  PCCLIM  ! NEG CHANGE QC DUE TO LIMITERS
     REAL, DIMENSION(KMS:KME) ::  PRELIM  ! NEG CHANGE QR DUE TO LIMITERS
     REAL, DIMENSION(KMS:KME) ::  QASUBCLIM ! NEG CHANGE QAW DUE TO LIMITERS
     REAL, DIMENSION(KMS:KME) ::  QAPRELIM ! NEG CHANGE QAR DUE TO LIMITERS
!bloss: These last six tendencies correspond to changes in cloud droplet or rain number
!    to keep the cloud/rain size within prescribed bounds.  These tendencies are more
!    artificial.
...
!bloss: 
!bloss(NOT NEEDED)     REAL, DIMENSION(KMS:KME) ::  QVPOSLIM  ! POS CHANGE QV DUE TO LIMITERS
! HM ADDED 12/13/06
...
!bloss: Add variables related to aerosol activation
      REAL :: NAD_NEW, NC_NEW
      REAL :: QAD_NEW, QAW_NEW, QACC_NEW

      !bloss: Add variables related to conserving aerosol number
      REAL :: NC_NEG, NR_NEG, NAD_NEG, NC_INCR

      !bloss: NCMAX_ACT is the maxmimum cloud droplet number
      !   that will keep the droplet size greater than the
      !   minimum size imposed by lammax
      REAL :: NCMAX_ACT, FAC_NCMAX, tmp_pgam, LAMC_MAX

!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC

      !bloss: We derive a maximum cloud droplet number concentration
      !  generated during activation to prevent newly nucleated drops
      !  from being smaller than the minimum size permitted in the
      !  RRTM radiation (where the minimum cloud droplet effective radius
      !  is 2.5 microns).  From the definition for effective radius,
      !   EFFC(K) = GAMMA(PGAM(K)+4.)/                        &
      !               GAMMA(PGAM(K)+3.)/LAMC(K)/2.*1.E6
      !  we find LAMC_MAX = 2E5*(PGAM(K)+4.).
      !  Next, we look at the limiting section of the code, and the
      !  maximum cloud droplet number can be computed from LAMC_MAX as:
      !     NCMAX = EXP( 3.*LOG(LAMC_MAX) +LOG(QC3D(K))              &
      !                 + LOG(GAMMA(PGAM(K)+1.)) -LOG(GAMMA(PGAM(K)+4.)) ) &
      !                 /CONS26
      !  Since the exp(log) is really just a product, this means
      !     NCMAX = LAMC_MAX**3 * GAMMA(PGAM(K)+1)/GAMMA(PGAM(K)+4) * QC3D(K)
      !  We can get a lower bound on this by assuming PGAM==10 (the upper limit)
      !  or a fixed value if dofix_pgam==true.  To make the computation simpler,
      !  we precompute FAC_NCMAX = LAMC_MAX**3 * GAMMA(PGAM(K)+1)/GAMMA(PGAM(K)+4)
      !  so that
      !     NCMAX = FAC_NCMAX*QC3D(K)
      IF(dofix_pgam) THEN
        tmp_pgam = pgam_fixed
      ELSE
        tmp_pgam = 10.
      END IF
      LAMC_MAX = 2.e5*(tmp_pgam+4.) ! This gives EFFC=2.5 micron
      FAC_NCMAX = EXP(3.*LOG(LAMC_MAX) &
                      + LOG(GAMMA(1.+tmp_pgam)) &
                      - LOG(GAMMA(4.+tmp_pgam)) ) / CONS26 

!!$      if(doMinimumActivatedDropSize) then
!!$        write(*,*) '===================================='
!!$        write(*,*) 'We are limiting the number of activated cloud droplets in thin clouds'
!!$        write(*,*) '   to prevent the formation of cloud droplets with reff<2.5 microns'
!!$        write(*,*) '   which is the lower limit of the cloud droplet effective radius in RRTMG.'
!!$        do k = 1,10
!!$          write(*,998) 10.**(-k),FAC_NCMAX*(10.**(-k))
!!$998       format('QC = ',E12.4,' kg/kg, NCMAX = ',E12.4)
!!$        end do
!!$        write(*,*) '===================================='
!!$      end if
...
!bloss: Initialize pgam and lamc to default values
         PGAM(KTS:KTE) = 10.3 ! default from Geoffroy et al (2010, ACP)
         LAMC(KTS:KTE) = (PGAM(KTS:KTE) + 1.) / 10.E-6
...
! INIT LIMITER PROFILES TO ZERO
            NSUBCLIM(KTS:KTE) = 0.  !bloss
            NSUBRLIM(KTS:KTE) = 0.  !bloss
            PCCLIM(KTS:KTE) = 0. 
            PRELIM(KTS:KTE) = 0. 
            QASUBCLIM(KTS:KTE) = 0.
            QAPRELIM(KTS:KTE) = 0.
            ...
            NADPOSLIM(KTS:KTE) = 0.
            NADNEGLIM(KTS:KTE) = 0.
!bloss(NOT NEEDED)            QVPOSLIM(KTS:KTE) = 0. 
...
!bloss(NOT NEEDED)                  QVPOSLIM(K) = QVPOSLIM(K)+QR3D(K)/DT !Limiter adds QR to QV
                  PRELIM(K) = PRELIM(K)-QR3D(K)/DT !Limiter zeros QR
                  NSUBRLIM(K) = NSUBRLIM(K)-NR3D(K)/DT !Limiter zeros NR
!!$                  NADPOSLIM(K) = NADPOSLIM(K)+NR3D(K)/DT !Limiter adds NR to NAD
                  QAPRELIM(K) = QAPRELIM(K)-QAR3D(K)/DT ! Limiter zeros QAR
                  ...
                  NR3D(K)=0.
               END IF
               IF ((QC3D(K).LT.1.E-8).AND.(ISATADJ.EQ.1)) THEN
!bloss(NOT NEEDED)                  QVPOSLIM(K) = QVPOSLIM(K)+QC3D(K)/DT !Limiter adds QC to QV
...
!!$                 QAWNEGLIM(K) = QAWNEGLIM(K)-QAW3D(K)/DT !Limiter zeros QAW
                 !bloss: Redefine these tendencies as physical processes
                 PCCLIM(K) = PCCLIM(K)-QC3D(K)/DT ! Evaporation of cloud liquid
                 NSUBCLIM(K) = NSUBCLIM(K)-NC3D(K)/DT ! decrease of cloud droplet number due to evap.
!!$                 NADPOSLIM(K) = NADPOSLIM(K)+NC3D(K)/DT ! decrease of cloud droplet number due to evap.
                 QASUBCLIM(K) = QASUBCLIM(K)-QAW3D(K)/DT ! decrease of cloud aerosol mass due to evap
...
! IF MIXING RATIO < QSMALL SET MIXING RATIO AND NUMBER CONC TO ZERO
            ...
            IF (QC3D(K).LT.QSMALL) THEN
               PCCLIM(K) = PCCLIM(K)-QC3D(K)/DT
               NSUBCLIM(K) = NSUBCLIM(K)-NC3D(K)/DT
               QASUBCLIM(K) = QASUBCLIM(K)-QAW3D(K)/DT
            ...
            IF (QR3D(K).LT.QSMALL) THEN
               PRELIM(K) = PRELIM(K)-QR3D(K)/DT
               NSUBRLIM(K) = NSUBRLIM(K)-NR3D(K)/DT
               QAPRELIM(K) = QAPRELIM(K)-QAR3D(K)/DT
...
! INITIALIZE SEDIMENTATION TENDENCIES FOR NUMBER CONCENTRATION
            ...
            NCSTEN(K) = 0.
            NRSTEN(K) = 0.
            NISTEN(K) = 0. !bloss(2020-10): complete set of sed. tendencies
            NSSTEN(K) = 0.
            NGSTEN(K) = 0.
...
! CASE FOR TEMPERATURE ABOVE FREEZING
            ...
            IF ((T3D(K).GE.TMELT).OR.(ILIQ.eq.1)) THEN
               ! ==== BEGIN WARM CLOUD ====
...
! BRNR 2/13/12 Add tracking of tendency due to this limiter
               ...
               ! NOTE: Prognostic aerosol budget not closed for ice phase
               NS3D(K) = MAX(0.,NS3D(K))
               NG3D(K) = MAX(0.,NG3D(K))

	       !bloss(2024-06-11): Make limiting for positivity conservative
               !                    for prognostic aerosol				    
               NC_NEG = MIN(0., NC3D(K))
               NC3D(K) = MAX(0.,NC3D(K)-NC_NEG) ! Max(0., ...) to avoid negatives like -1.e-32
               NCPOSLIM(K) = NCPOSLIM(K)-NC_NEG/DT
               
               NR_NEG = MIN(0., NR3D(K))
               NR3D(K) = MAX(0.,NR3D(K)-NC_NEG)
               NRPOSLIM(K) = NRPOSLIM(K)-NR_NEG/DT
               
               IF (IPRGAER.EQ.1) THEN
                  ! Decrease NAD3D(K) --- unactivated aerosol --- to balance hole-filling of
                  !   any (probably rare) negative values in NC3D(K) and NR3D(K).
                  NAD3D(K) = NAD3D(K) + NC_NEG + NR_NEG
                  NADNEGLIM(K) = NADNEGLIM(K)+NC_NEG/DT+NC_NEG/DT

                  ! Make sure NAD3D(K) is non-negative
                  NAD_NEG = MIN(0., NAD3D(K))
                  NAD3D(K) = MAX(0.,NAD3D(K)-NAD_NEG)
                  NADPOSLIM(K) = NADPOSLIM(K)-NAD_NEG/DT

                  ! If NAD3D(K) was negative, balance hole-filling by decreasing
                  !   NC3D(K) as long as this does not drive NC3D(K) to be negative
                  NC_INCR = MAX(-NC3D(K),NAD_NEG)
                  NC3D(K) = MAX(0., NC3D(K) + NC_INCR)
                  NCNEGLIM(K) = NCNEGLIM(K)+NC_INCR/DT
...
! Brnr modify the bound check routines to conserve Na+Nc
                  ...
                  IF (LAMC(K).LT.LAMMIN) THEN
                     ...
                     IF (IPRGAER.EQ.1) THEN
                        ...
                        NC3D(K) = MAX(0.1*DUM, MIN(DUM, DUM1+DUM2) ) ! cloud will be at most 10^(1/3)*60 microns
                  ...
                  ELSE IF (LAMC(K).GT.LAMMAX) THEN
                     ...
                     IF (IPRGAER.EQ.1) THEN
                        ...
                        DUM = EXP(3.*LOG(LAMC(K))+LOG(QC3D(K))+              &
                             LOG(GAMMA(PGAM(K)+1.))-LOG(GAMMA(PGAM(K)+4.)))/CONS26
                        !bloss(2018-02): prevent creation of number
                        !bloss(2019-03): probably do not have to worry about Nc-->0 here
...
!             DUM = -CPW/XLF(K)*T3D(K)*PRACS(K)
                 DUM = -CPW/XLF(K)*MAX(0.,T3D(K)-273.15)*PRACS(K)
...
!                    SC(K)**(1./3.)*CONS10/                   &
!                   (LAMS(K)**CONS35))+DUM
                 PSMLT(K)=2.*PI*N0S(K)*KAP(K)*MIN(0.,TMELT-T3D(K))/       &
...
!             DUM = -CPW/XLF(K)*T3D(K)*PRACG(K)
                 DUM = -CPW/XLF(K)*MAX(0.,T3D(K)-273.15)*PRACG(K)
...
!                    SC(K)**(1./3.)*CONS11/                   &
!                   (LAMG(K)**CONS36))+DUM
             PGMLT(K)=2.*PI*N0G(K)*KAP(K)*MIN(0.,TMELT-T3D(K))/ 		 &
...
! BASED ON TWOMEY 1959
                              ...
                              if(doMinimumActivatedDropSize) then
                                ! NC3D>NCMAX_ACT will drive the cloud droplet size below the threshold of ~1um.
                                ! limit aerosol activation to keep NC3D<=NCMAX_ACT
                                NCMAX_ACT = FAC_NCMAX*(QC3D(K)+QC3DTEN(K)*DT) 
                                DUM2 = MIN(DUM2, NCMAX_ACT)
                              end if

                              ! Activation tendency = (new NC - old NC)/DT
                              DUM2 = (DUM2-NC3D(K))/DT
                              DUM2 = MAX(0.,DUM2)
...
! DROPLET ACTIVATION FROM ABDUL-RAZZAK AND GHAN (2000)
                              ...
                              IF ((NANEW1.GT.0.AND.RM1.GT.0).OR.(NANEW2.GT.0.AND.RM2.GT.0)) THEN
                                 ...
                                 IF (NANEW2.GT.0.AND.RM2.GT.0) THEN
...
! MAKE SURE THIS VALUE ISN'T GREATER THAN TOTAL NUMBER OF AEROSOL
                              ...
                              ! Activation tendency = (new NC - old NC)/DT
                                 ...
                                 IF (IPRGAER.EQ.1) THEN
                                    ...
                                    !bloss(2020-11): Consider wet+dry aerosol as a single lognormal size distribtuion
                                    !  in which the smallest particles are dry and the largest wet (i.e., inside cloud droplets).

                                    ! first get the new paritioning of number mixing ratios
                                    NAD_NEW = MAX(0., NAD3D(K) + NAD3DTEN(K)*DT)
                                    NC_NEW = MAX(0., NC3D(K) + NC3DTEN(K)*DT)

                                    ! find out how much aerosol mass got transferred from the second aerosol mode
                                    DUM2 = MAER2/2.*(1.-DERF1(UU2-3.*SQRT(2.)/2.*LOG(SIG2)))
                                    DUM2 = DUM2/RHO(K) ! convert to kg/kg
                                    DUM2 = MIN(MAER2/RHO(K),DUM2) ! Make sure doesn't exceed available aerosol mass
                                    QAW3DTEN(K) = QAW3DTEN(K) + DUM2/DT

                                    ! Get the new wet+dry aerosol mass mixing ratio (including mode two activation)
                                    QACC_NEW = MAER1/RHO(K) + DUM2 
                                    
                                    ! get mass paritioning between wet and dry based on the fraction of aerosol number
                                    !   that is dry (i.e. interstitial aerosol) and the geometric standard deviation
                                    QAD_NEW = DryAerosolMassFraction( &
                                         NAD_NEW / MAX( EPSILON(1.), NAD_NEW + NC_NEW ) , SIG1 , 3 ) &
                                         * QACC_NEW

                                    ! New wet aerosol mass mixing ratio
                                    QAW_NEW = MAX(0., QACC_NEW - QAD_NEW )

                                    !bloss: We only need to worry about total accumulation mode mass here, 
                                    !  so just track mode 2 aerosol mass that gets activated.
!bloss                                    DUM1 = MAER1/2.*(1.-DERF1(UU1-3.*SQRT(2.)/2.*LOG(SIG1)))
!bloss                                    DUM2 = MAER2/2.*(1.-DERF1(UU2-3.*SQRT(2.)/2.*LOG(SIG2)))
!bloss                                    DUM2 = (DUM1 + DUM2)/RHO(K)
!bloss                                    DUM2 = MIN((MAER1+MAER2)/RHO(K),DUM2) ! Make sure doesn't exceed total aerosol mass
!bloss                                    DUM2 = (DUM2-QAW3D(K))/DT
!bloss                                    DUM2 = MAX(0.,DUM2) ! Make sure activation doesn't remove mass
                                    
                                    !bloss(2020-11): update these based on new computations above
                                    QACT(K) = MAX(0., QAW_NEW - QAW3D(K) )/DT !brnr track aerosol mass activation tendency
                                    QAW3DTEN(K) = QAW3DTEN(K) + (QAW_NEW - QAW3D(K))/DT
                                    QAD3DTEN(K) = QAD3DTEN(K) + (QAD_NEW - QAD3D(K))/DT
                                 END IF  ! IPRGAER
...
! AEROSOL MASS CALC FOR NON-EQUIL. SS, BOUND CHECK, AND TENDENCY
                                    IF (IPRGAER.EQ.1) THEN
                                       ...
                                       !   that is dry (i.e. interstitial aerosol) and the geometric standard deviation
                                       QAD_NEW = DryAerosolMassFraction( &
                                            NAD_NEW / MAX( EPSILON(1.), NAD_NEW + NC_NEW ) , SIG1 , 4 ) &
                                            * QACC_NEW
...
!bloss(2020-11)                                       DUM1 = MAER1/2.*(1.-DERF1(UU1-3.*SQRT(2.)/2.*LOG(SIG1)))
!bloss(2020-11)                                       DUM2 = MAER2/2.*(1.-DERF1(UU2-3.*SQRT(2.)/2.*LOG(SIG2)))
!bloss(2020-11)                                       DUM2 = (DUM1 + DUM2)/RHO(K)
!bloss(2020-11)                                       DUM2 = MIN((MAER1+MAER2)/RHO(K),DUM2) ! Make sure doesn't exceed total aerosol mass
!bloss(2020-11)                                       DUM2 = MIN(DUM2,DUMACTM)
!bloss(2020-11)                                       DUM2 = (DUM2-QAW3D(K))/DT
!bloss(2020-11)                                       DUM2 = MAX(0.,DUM2)
!bloss(2020-11)                                       QACT(K) = DUM2 !brnr track aerosol mass activation tendency
!bloss(2020-11)                                       QAW3DTEN(K) = QAW3DTEN(K)+DUM2
!bloss(2020-11)                                       QAD3DTEN(K) = QAD3DTEN(K)-DUM2
                                    END IF  ! IPRGAER
                                 END IF ! (NANEW1.GT.0).OR.(NANEW2.GT.0)
                              END IF ! IACT
                        ...
                        ELSE IF (IACT.EQ.2) THEN
                           ...
                           IF ((NANEW1.GT.0.AND.RM1.GT.0).OR.(NANEW2.GT.0.AND.RM2.GT.0)) THEN
                              ...
                              IF (IPRGAER.EQ.1) THEN
                                 ...
                                 !   that is dry (i.e. interstitial aerosol) and the geometric standard deviation
                                 QAD_NEW = DryAerosolMassFraction( &
                                      NAD_NEW / MAX( EPSILON(1.), NAD_NEW + NC_NEW ) , SIG1 , 5 ) &
                                      * QACC_NEW
...
!bloss(2020-11)                                 DUM2 = MIN((MAER1+MAER2)/RHO(K),DUM2) ! Make sure doesn't exceed total aerosol mass
!bloss(2020-11)                                 DUM2 = (DUM2-QAW3D(K))/DT
!bloss(2020-11)                                 DUM2 = MAX(0.,DUM2) ! Make sure activation doesn't remove mass
!bloss(2020-11)                                 QACT(K) = DUM2 !brnr track activated mass tendency
!bloss(2020-11)                                 QAW3DTEN(K) = QAW3DTEN(K)+DUM2
!bloss(2020-11)                                 QAD3DTEN(K) = QAD3DTEN(K)-DUM2
                              END IF  ! IPRGAER
                           END IF ! (NANEW1.GT.0).OR.(NANEW2.GT.0)
...
!brnr: make the Nc tendency due to evaporation switchable from prm file
            ...
            IF ((PCC(K).LT.0.).AND.(ISATADJ.EQ.0)) THEN
               IF (IEVPNC.EQ.1) THEN
                  DUM = PCC(K)*DT/QC3D(K)
                  !DUM = MAX(-1.,DUM)
...
! ACTIVATION OF CLOUD DROPLETS
       ...
       !bloss(2024-06-05): Set up variables for accumulation mode number (NANEW1, #/m3)
       !   and accumulation mode mass (MAER1, kg/m3).  Placeholders for an additional
       !   aerosol category (NANEW2, MAER2) are included but not used.
       IF (IPRGAER.EQ.1) THEN 
          NANEW1 = (NAD3D(K) + NC3D(K))*RHO(K)
          NANEW2 = 0. !Placeholder for further dry aerosol modes !brnr
          MAER1 = (QAD3D(K) + QAW3D(K))*RHO(K)
...
! ADD FALLOUT TERMS TO EULERIAN TENDENCIES
    ...
    NISTEN(K) = NISTEN(K)-FALTNDNI/NSTEPI/RHO(k)
...
! ADD FALLOUT TERMS TO EULERIAN TENDENCIES
    ...
    NSSTEN(K) = NSSTEN(K)-FALTNDNS/NSTEPS/RHO(k)
...
! ADD FALLOUT TERMS TO EULERIAN TENDENCIES
    ...
    NGSTEN(K) = NGSTEN(K)-FALTNDNG/NSTEPG/RHO(k)
...
!bloss(NOT NEEDED)            QVPOSLIM(K) = QVPOSLIM(K)+QC3D(K)/DT !Limiter adds QC to QV
            PCCLIM(K) = PCCLIM(K)-QC3D(K)/DT !Limiter zeros QC
            QV3D(K)=QV3D(K)+QC3D(K)
            T3D(K)=T3D(K)-QC3D(K)*XXLV(K)/CPM(K)
            QC3D(K)=0.
         END IF
...
!bloss: Add limiting tendencies to total microphysical tendencies
!   so that budgets will include this output
   QV3DTEN(:) = QV3DTEN(:) - PRELIM(:) - PCCLIM(:)
   QC3DTEN(:) = QC3DTEN(:) + PCCLIM(:)
   QR3DTEN(:) = QR3DTEN(:) + PRELIM(:)

   NAD3DTEN(:) = NAD3DTEN(:) - NSUBCLIM(:) - NSUBRLIM(:) + NADPOSLIM(:) + NADNEGLIM(:)
   NC3DTEN(:) = NC3DTEN(:) + NSUBCLIM(:) + NCPOSLIM(:) + NCNEGLIM(:)
   NR3DTEN(:) = NR3DTEN(:) + NSUBRLIM(:) + NRPOSLIM(:) + NRNEGLIM(:)

   QAD3DTEN(:) = QAD3DTEN(:) - QASUBCLIM(:) - QAPRELIM(:)
   QAW3DTEN(:) = QAW3DTEN(:) + QASUBCLIM(:)
   QAR3DTEN(:) = QAR3DTEN(:) + QAPRELIM(:)

   !bloss: store sedimentation tendencies in stend array
   !  with an order that matches the order of the fields in
   !  micro_field(:,:,:,n)

   stend(:,:) = 0.
   ! water mass
   if(iqcl.gt.0) stend(:,iqcl) = QCSTEN(:)
   if(iqr.gt.0) stend(:,iqr) = QRSTEN(:)
   if(iqci.gt.0) stend(:,iqci) = QISTEN(:)
   if(iqs.gt.0) stend(:,iqs) = QNISTEN(:)
   if(iqg.gt.0) stend(:,iqg) = QGSTEN(:)

   ! hydrometeor number
   if(incl.gt.0) stend(:,incl) = NCSTEN(:)
   if(inr.gt.0) stend(:,inr) = NRSTEN(:)
   if(inci.gt.0) stend(:,inci) = NISTEN(:)
   if(ins.gt.0) stend(:,ins) = NSSTEN(:)
   if(ing.gt.0) stend(:,ing) = NGSTEN(:)

   ! aerosol mass
   if(iqaw.gt.0) stend(:,iqaw) = QAWSTEN(:)
   if(iqar.gt.0) stend(:,iqar) = QARSTEN(:)

   !NOTE: neither vapor nor dry aerosol sediment


      if(do_proc_extra.AND.(n_proc_extra.gt.0)) then
        proc_extra(:,1) = prc(:)
        proc_extra(:,2) = nprc(:)
        proc_extra(:,3) = nprc1(:)
        proc_extra(:,4) = pra(:)
      ...
      if(do_accumulate_micro_proc_rates) then
        ...
        idx = idx + 1
        micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + PCCLIM ! CHANGE IN CLOUD LIQUID MASS DUE TO EVAPORATION OF CELLS WITH SMALL QC
        idx = idx + 1
        micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + PRELIM ! CHANGE IN RAIN MASS DUE TO NEGATIVE LIMITING

        ! ======== WARM CLOUD NUMBER PROCESS RATES ==========
        idx = idx + 1
        ...
        idx = idx + 1
        micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + NACT(:) ! CHANGE IN CLOUD DROPLET NUMBER DUE TO DROPLET ACTIVATION
        idx = idx + 1
        micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + NRSTEN(:) ! CHANGE IN RAIN NUMBER DUE TO SEDIMENTATION
        idx = idx + 1
        micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + NCSTEN(:) ! CHANGE IN CLOUD DROPLET NUMBER DUE TO SEDIMENTATION
        idx = idx + 1
        micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + NSUBCLIM ! LOSS OF NC DURING EVAP IN GRID CELLS WITH VERY SMALL QC
        idx = idx + 1
        micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + NSUBRLIM ! LOSS OF NR DURING EVAP IN GRID CELLS WITH VERY SMALL QR
        idx = idx + 1
        micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + NCPOSLIM ! CHANGE IN CLOUD DROPLET NUMBER DUE TO POSITIVE LIMITING
        idx = idx + 1
        micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + NCNEGLIM ! CHANGE IN CLOUD DROPLET NUMBER DUE TO NEGATIVE LIMITING
        idx = idx + 1
        micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + NRPOSLIM ! CHANGE IN RAIN DROPLET NUMBER DUE TO POSITIVE LIMITING
        idx = idx + 1
        micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + NRNEGLIM ! CHANGE IN RAIN DROPLET NUMBER DUE TO NEGATIVE LIMITING


        if(iprgaer.eq.1) then
          !bloss(2019-07-22): Output tendencies related to prognostic aerosol

          ! ======== PROGNOSTIC AEROSOL MASS PROCESS RATES ==========
          idx = idx + 1
          micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + QAPRA(:) ! CHANGE IN RAIN AEROSOL MASS DUE TO ACCRETION
          idx = idx + 1
          micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + QAPRC(:) ! CHANGE IN RAIN AEROSOL MASS DUE TO AUTOCONVERSION
          idx = idx + 1
          micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + QAPRE(:) ! CHANGE IN RAIN AEROSOL MASS DUE TO RAIN EVAPORATION
          idx = idx + 1
          micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + QASUBC(:) ! CHANGE IN CLOUD AEROSOL MASS DUE TO CLOUD DROPLET EVAPORATION
          idx = idx + 1
          micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + QACT(:) ! CHANGE IN CLOUD AEROSOL MASS DUE TO DROPLET ACTIVATION
          idx = idx + 1
          micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + QASUBCLIM ! CHANGE IN CLOUD AEROSOL MASS DUE TO NEGATIVE LIMITING
          idx = idx + 1
          micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + QAPRELIM ! CHANGE IN RAIN AEROSOL MASS DUE TO NEGATIVE LIMITING

          ! ======== PROGNOSTIC AEROSOL NUMBER PROCESS RATES ==========
          idx = idx + 1
          micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + NADPOSLIM ! CHANGE IN DRY AEROSOL NUMBER DUE TO POSITIVE LIMITING
          idx = idx + 1
          micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + NADNEGLIM ! CHANGE IN DRY AEROSOL NUMBER DUE TO NEGATIVE LIMITING
        end if

        if(doShipDilution) then
          idx = idx + 1
          micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + NACC3DTEN! CHANGE IN ACCUM AEROSOL NUMBER DUE TO SHIP DILUTION
          idx = idx + 1
          micro_proc_rates(:,idx) = micro_proc_rates(:,idx) + QACC3DTEN !CHANGE IN ACCUM AEROSOL MASS DUE TO SHIP DILUTION
        end if


        if(iliq.eq.0) then

          ! ======== COLD CLOUD MASS PROCESS RATES ==========
          idx = idx + 1
          ...
          idx = idx + 1          ! hm new process rate output
