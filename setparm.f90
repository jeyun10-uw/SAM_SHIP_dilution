...
NAMELIST /PARAMETERS/ dodamping, doupperbound, docloud, doprecip, &
		...
		CEM,LES,OCEAN,LAND,SFC_FLX_FXD,SFC_TAU_FXD, soil_wetness, &
	        ...
	        doseasons, doperpetual, doradhomo, dosfchomo, doisccp, &
                ...
                ocean_flux_option, & !bloss: option for COARE3.6 surface fluxes
...
NAMELIST /UWOPTIONS/ rad_simple_fluxdiv1, &
     ...
     lambda_ship_track, lambda_cf, print_lambda, &
     dowtg_qnudge, itau_wtg_qnudge, &
     dowtg_tnudge, itau_wtg_tnudge,  taulz_wtg_tnudge, &
     tauz0_wtg_qnudge, taulz_wtg_qnudge, &
     am_wtg, am_wtg_exp, lambda_wtg, lambda_wtg_min, wtg_start_time, wtg_start_ramp, wtg_end_time, wtg_end_ramp, pres_trop, lambda_ship_track_start_time, lambda_ship_track_rate, &
     nudge_to_sounding_winds, &
     doSmoothDamping, zbot_SmoothDamping, tau_SmoothDamping, &
     doenforce_cgils_qfloor, ztop_qfloor, qfloor, tau_qfloor, &
     donudging_transient, transient_nudging_start, transient_nudging_end, &
     tau_transient_nudging, transient_nudging_ramp
...
!  Set parameters 
	...
	notopened3D = .true.
        ...
        ALLOCATE(condavg_factor(nzm,ncondavg), & ! replaces old cloud_factor, core_factor
            ...
            ! zb_old(nzm,ncondavg), & !old inversion base heights for tendencies
             STAT=ierr)
        if(ierr.ne.0) then
             write(*,*) '**************************************************************************'
        ...
        icondavg_sh_cloud = -1
        icondavg_sh_clear = -1
        icondavg_no_sh_cloud = -1
        icondavg_no_sh_clear = -1
        ...
        if(doShipTrackConditionals) then
           icondavg = icondavg + 1
           condavgname(icondavg) = 'ST1'
           condavglongname(icondavg) = 'ship track with cloud'
           icondavg_sh_cloud = icondavg

           icondavg = icondavg + 1
           condavgname(icondavg) = 'ST2'
           condavglongname(icondavg) = 'ship track without cloud'
           icondavg_sh_clear = icondavg

           icondavg = icondavg + 1
           condavgname(icondavg) = 'ST3'
           condavglongname(icondavg) = 'no ship track with cloud'
           icondavg_no_sh_cloud = icondavg

           icondavg = icondavg + 1
           condavgname(icondavg) = 'ST4'
           condavglongname(icondavg) = 'no ship track without cloud'
           icondavg_no_sh_clear = icondavg
        end if
